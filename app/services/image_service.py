...
class ImageService:
    ...
    async def get_ingredient_suggestions(self, detected_ingredients: List[str]) -> Dict[str, Any]:
        ...
        has_vegetables = any(ing in detected_ingredients for ing in ['carrot', 'potato', 'onion', 'tomato', 'spinach', 'mixed vegetables'])
        has_protein = any(ing in detected_ingredients for ing in ['chicken', 'fish', 'egg', 'meat', 'paneer'])
        has_grains = any(ing in detected_ingredients for ing in ['rice', 'wheat', 'bread', 'pasta'])
