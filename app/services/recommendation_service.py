...
class RecommendationService:
    ...
    async def get_meal_recommendations(
        self, whatsapp_id: str, user_message: str, max_results: int = 3
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            recommendations = await self._generate_recommendations(
                user, user_message, intent, max_results
            )
    ...
    async def _generate_recommendations(
        self, user: Dict[str, Any], message: str, intent: str, max_results: int
    ) -> List[Dict[str, Any]]:
        ...
        if intent == "MOOD":
            return await self._handle_mood_request(
                message, search_criteria, max_results
            )
            ...
            return await self._handle_pantry_request(
                message, search_criteria, max_results
            )
        elif intent == "DIETARY_QUERY":
            return await self._handle_dietary_request(
                message, search_criteria, max_results
            )
        elif intent == "PLANWEEK":
            return await self._handle_weekly_plan_request(
                search_criteria, max_results * 2
            )
    ...
    async def _handle_mood_request(
        ...
        return self._format_recommendations(
            meals[:max_results],
            f"Based on your mood for {search_query or 'comfort'} food",
        )
    ...
    async def _handle_recipe_request(
        ...
        return self._format_recommendations(
            meals[:max_results], f"Recipe suggestions for '{dish_name}'"
        )
    ...
    async def _handle_pantry_request(
        ...
        for meal in meals:
            ...
            if any(
                ingredient.lower() in " ".join(meal_ingredients)
                ...
                meal["ingredient_match_score"] = sum(
                    ...
                    for ingredient in ingredients
                    if any(
                        ingredient.lower() in meal_ing for meal_ing in meal_ingredients
                    )
                )
        ...
        matching_meals.sort(
            key=lambda x: x.get("ingredient_match_score", 0), reverse=True
        )

        ingredients_text = ", ".join(ingredients)
        return self._format_recommendations(
            matching_meals[:max_results], f"Using your ingredients: {ingredients_text}"
        )
    ...
    async def _handle_dietary_request(
        ...
        return self._format_recommendations(
            meals[:max_results], f"Meals for your {dietary_filter} preference"
        )
    ...
    async def _handle_general_request(
        ...
        suggestion_text = f"Here are some meal suggestions" + (
    ...
    def _prioritize_by_tags(
        ...
        scored_meals.sort(
            key=lambda x: (x.get("tag_score", 0), random.random()), reverse=True
        )
        return scored_meals
    ...
    def _format_recommendations(
        self, meals: List[Dict[str, Any]], context: str
    ) -> List[Dict[str, Any]]:
